    ...
    (async () => {
      ...
      if (data) {
        ...
        setInitial({
          ...
          occupation: Array.isArray((data as any).occupation)
            ? ((data as any).occupation as string[])
            : (data as any).occupation
              ? [(data as any).occupation as string]
              : [],
          occupational_hazards: Boolean((data as any).occupational_hazards),
          hazard_physical: Boolean((data as any).hazard_physical),
          hazard_biological: Boolean((data as any).hazard_biological),
          hazard_chemical: Boolean((data as any).hazard_chemical),
          shift_work: Boolean((data as any).shift_work),
          high_stress_environment: Boolean((data as any).high_stress_environment),
...
function DialogShell({ patientId, patientName, open, onOpenChange, onCompleted }: Props) {
  ...
  const persist = async (
    ...
    const payload: Record<string, unknown> = {
      ...
      occupational_hazards: nextForm.occupational_hazards,
      hazard_physical: nextForm.hazard_physical,
      hazard_biological: nextForm.hazard_biological,
      hazard_chemical: nextForm.hazard_chemical,
      shift_work: nextForm.shift_work,
      high_stress_environment: nextForm.high_stress_environment,
