...
import { blankMole, type MoleEntry } from "@/components/patients/onboarding/OnboardingFormContext";
import { FieldLabel } from "@/components/patients/onboarding/shared";
...
export function regionForPin(side: "front" | "back", x: number, y: number): string {
  ...
  if (y < 22) {
    return side === "front"
      ? lateral === "center" ? "Chest (center)" : `Chest (${lateral})`
...
export interface MolesPanelProps {
  moles: MoleEntry[];
  onChange: (moles: MoleEntry[]) => void;
}

export function MolesPanel({ moles, onChange }: MolesPanelProps) {
  const [highlightId, setHighlightId] = useState<string | null>(null);
  const cardRefs = useRef<Record<string, HTMLDivElement | null>>({});

  const updateMole = (id: string, partial: Partial<MoleEntry>) => {
    onChange(moles.map((m) => (m.id === id ? { ...m, ...partial } : m)));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <BodySilhouette side="front" moles={moles} onAdd={(x, y) => addMoleAt("front", x, y)} onPinClick={focusMole} />
        <BodySilhouette side="back" moles={moles} onAdd={(x, y) => addMoleAt("back", x, y)} onPinClick={focusMole} />
      </div>
        ...
        {moles.length === 0 ? (
          ...
          moles.map((mole, idx) => (
            <div
              ...
              className={cn("transition-all", highlightId === mole.id && "ring-2 ring-primary rounded-xl")}
            >
              <MoleCard
                index={idx + 1}
                mole={mole}
...
function BodySilhouette({
  ...
  return (
    <div className="rounded-xl border border-border bg-card/40 p-3 flex flex-col items-center">
      ...
      >
        <img src={src} alt="" draggable={false} className="pointer-events-none absolute inset-0 h-full w-full object-contain" />
        {sidePins.map(({ mole, number }) => (
          <button
            key={mole.id}
            type="button"
            onClick={(ev) => { ev.stopPropagation(); onPinClick(mole.id); }}
            ...
            style={{ left: `${mole.pin_x}%`, top: `${mole.pin_y}%`, width: 18, height: 18 }}
          >
            {number}
          </button>
        ))}
    ...
    <div className="rounded-xl border border-border bg-card/40 px-4 py-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span className="text-[10px] uppercase tracking-wide text-muted-foreground">· {mole.side}</span>
        </div>
        <Button type="button" variant="ghost" size="icon" onClick={() => setEditingLocation((v) => !v)} aria-label="Edit location label" className="h-8 w-8 text-muted-foreground hover:text-foreground">
          <Pencil className="h-4 w-4" />
        </Button>
        <Button type="button" variant="ghost" size="icon" onClick={onRemove} aria-label="Remove mole" className="h-8 w-8 text-muted-foreground hover:text-destructive">
      ...
      <div className="grid grid-cols-3 gap-3">
        {(Object.entries(ABCDE_OPTIONS) as [keyof typeof ABCDE_OPTIONS, string[]][]).map(([key, opts]) => (
          <div key={key}>
            ...
            <Select value={mole[key]} onValueChange={(v) => onChange({ [key]: v } as Partial<MoleEntry>)}>
...
function ImageThumb({ file, onRemove }: { file: File; onRemove: () => void }) {
  ...
  if (typeof window !== "undefined" && url === null) {
    try { setUrl(URL.createObjectURL(file)); } catch { /* ignore */ }
  }
  return (
